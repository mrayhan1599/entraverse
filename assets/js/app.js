...
function calculateElapsedPeriodDays(startDate, endDate, referenceDate = new Date()) {
  const start = normalizeDateOnly(startDate);
  const end = normalizeDateOnly(endDate);
  ...
  const clampedReference =
    referenceTime < start.getTime()
      ? start
      : referenceTime > end.getTime()
      ? end
      : reference;

  return calculateInclusiveDays(start, clampedReference);
}
...
async function ensureWarehouseAverageData({ force = false } = {}) {
  ...
  const periodADays = periodA?.elapsedDays || periodA?.days || calculateInclusiveDays(periodA?.start, periodA?.end) || DAILY_AVERAGE_WINDOW_DAYS;
  const periodBDays = periodB?.elapsedDays || periodB?.days || calculateInclusiveDays(periodB?.start, periodB?.end) || DAILY_AVERAGE_WINDOW_DAYS;
...
function setupTopbarBrandNavigation() {
...
function getWarehouseAutoPeriods(referenceDate = new Date()) {
  ...
  const periodATotalDays = calculateInclusiveDays(periodAStart, periodAEnd);
  const periodBTotalDays = calculateInclusiveDays(periodBStart, periodBEnd);
  const periodAElapsedDays =
    calculateElapsedPeriodDays(periodAStart, periodAEnd, referenceDateOnly) ?? periodATotalDays;
  const periodBElapsedDays =
    calculateElapsedPeriodDays(periodBStart, periodBEnd, referenceDateOnly) ?? periodBTotalDays;
...
function ensurePurchaseAuthorizationPath(integration) {
  if (!integration || typeof integration !== 'object') {
    return integration;
  }

  const hasAuthorizationPath = (integration.authorizationPath || '').toString().trim();
  if (hasAuthorizationPath) {
    return integration;
  }

  return { ...integration, authorizationPath: 'partner/core' };
}
...
function extractPurchaseDocumentItems(record) {
  ...
  const candidates = [
    ...
    record.transactionLines,
    record.transaction_lines_attributes,
...
async function fetchMekariPurchaseDocuments(
  ...
  const purchaseIntegration = ensurePurchaseAuthorizationPath(integration);
  const { token } = resolveMekariApiDetails(purchaseIntegration);
  if (!token) {
    throw new Error('Token API Mekari Jurnal belum tersedia. Perbarui pengaturan integrasi.');
  }
  ...
  const url = buildMekariAuthorizedUrl(
    purchaseIntegration,
    `${buildPurchaseDocumentPath(config.endpoint)}?${params.toString()}`
  );
  const headers = new Headers({ Accept: 'application/json' });
  headers.set('Authorization', token);
...
function renderProcurementTable(plan = [], filterText = '') {
  ...
  const rowMarkup = rows
    .map(entry => {
      ...
      const availableStockDisplay =
        roundedAvailableStock !== null && roundedAvailableStock !== undefined
          ? formatNumber(roundedAvailableStock)
          : '—';
      ...
      return `
        <tr>
          ...
          <td>
            <div class="table-primary">${escapeHtml(availableStockDisplay)}</div>
          </td>
          <td>
