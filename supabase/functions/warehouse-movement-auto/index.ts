...
function normalizeWarehouseSummary(
  ...
  const header = summary?.header && typeof summary.header === "object"
    ? {
        date: (summary.header as JsonRecord).date?.toString() ?? null,
        currency: (summary.header as JsonRecord).currency?.toString() ?? null,
        companyName: (summary.header as JsonRecord).company_name?.toString() ?? null
      }
    : null

  const list = Array.isArray(summary?.list) ? (summary.list as WarehouseEntry[]) : []
