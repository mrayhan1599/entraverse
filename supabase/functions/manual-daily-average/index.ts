...
async function upsertProducts(client: SupabaseClient, payload: JsonRecord[], { chunkSize = 50 } = {}) {
  const items = Array.isArray(payload) ? payload.filter(Boolean) : []
  if (!items.length) return

  const safeChunkSize = Math.max(1, Math.floor(Number(chunkSize) || 50))
  for (let index = 0; index < items.length; index += safeChunkSize) {
    const slice = items.slice(index, index + safeChunkSize)
...
Deno.serve(async req => {
  ...
  try {
    ...
    await upsertProducts(supabase, updates)
