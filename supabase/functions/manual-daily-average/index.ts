...
function calculateFinalDailyAverage({
  ...
  if (!hasTermA && !hasTermB) return null

  const safeAverageA = hasTermA && Number.isFinite(averageA) ? Number(averageA) : 0
  const safeFactorA = hasTermA && Number.isFinite(factorA) ? Number(factorA) : 0
  const safeAverageB = hasTermB && Number.isFinite(averageB) ? Number(averageB) : 0
  const safeFactorB = hasTermB && Number.isFinite(factorB) ? Number(factorB) : 0
