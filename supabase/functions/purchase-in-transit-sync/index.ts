...
function normalizeStatus(value: unknown) {
  ...
  if (["unpaid", "belum dibayar", "pending"].includes(raw)) return "unpaid"
...
function normalizePurchaseOrder(entry: Record<string, unknown>): PurchaseOrder | null {
  ...
  const items = Array.isArray(entry?.items)
    ? (entry.items as Record<string, unknown>[]).map(item => normalizePurchaseOrderItem(item)).filter(Boolean)
    : []
...
function normalizePurchaseOrderItem(entry: Record<string, unknown>): PurchaseOrderItem | null {
  const sku = (entry?.sku ?? entry?.product_code ?? entry?.item_code ?? "").toString().trim()
  if (!sku) return null
...
Deno.serve(async req => {
  ...
  try {
    ...
    const orders = await fetchPurchaseOrders({ baseUrl: url, path, accessToken })
