...
const DEFAULT_STATUS_QUERY_KEY = "transaction_status"
...
function normalizeStatus(value: unknown) {
  ...
  // Any unrecognized status should still be treated as unpaid so it passes
  // the status constraint in the purchase_orders table and keeps contributing
  // to in-transit calculations until explicitly marked paid/cancelled.
  return "unpaid"
}
...
async function fetchPurchaseOrders({
  ...
  while (true) {
    ...
    url.searchParams.set(statusQueryKey || DEFAULT_STATUS_QUERY_KEY, statusQueryValue || DEFAULT_STATUS_QUERY_VALUE)
    ...
    if (normalized.length < pageSize || (total && results.length >= total)) {
      break
    }
