...
async function removeOlderProcurements(client: SupabaseClient, dueRows: ProcurementDueRow[]) {
  ...
  for (const row of dueRows) {
    ...
    if (!existing || existing.next_procurement_date < row.next_procurement_date) {
      pairs.set(key, row)
    }
  }
  ...
  for (const row of pairs.values()) {
    const { error: deleteError } = await client
      ...
      .delete()
      .or(
        `next_procurement_date.lte.${row.next_procurement_date},required_stock.lte.0,next_procurement_signature.neq.${row.next_procurement_signature}`
      )
...
async function updateProducts(client: SupabaseClient, products: ProductRecord[], periods: ProcurementPeriod[], today: Date) {
  ...
  const zeroProcurementPairs = new Set<string>()
    ...
    const updatedRows = rawRows.map(row => {
      ...
      if (isSameDate(procurementDate, today)) {
        ...
        if (requiredStock !== null && requiredStock > 0) {
          ...
          const key = `${record.id}::${resolveVariantId(row as VariantPricingRow)}`
          zeroProcurementPairs.add(key)
        }
      }
  ...
  if (zeroProcurementPairs.size) {
    const zeroPairList = Array.from(zeroProcurementPairs).map(key => {
      const [product_id, variant_id] = key.split("::")
      return { product_id, variant_id }
    })

    for (const pair of zeroPairList) {
      const { error: deleteError } = await client
        .from("procurement_due")
        .delete()
        .eq("product_id", pair.product_id)
        .eq("variant_id", pair.variant_id)
        .or("required_stock.lte.0,required_stock.is.null")

      if (deleteError) {
        throw new Error(`Failed to delete zero-quantity procurement entries: ${deleteError.message}`)
      }
    }
  }
