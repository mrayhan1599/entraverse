...
function deriveProcurementPlans(products: ProductRecord[], today = new Date()): ProcurementPlan[] {
  ...
  const targetPeriod = periods.find(period => period.start >= today) ?? periods[periods.length - 1]
  ...
  for (const product of products) {
    ...
    variants.forEach(variant => {
      ...
      const plan = computeProcurementPlan(variant, targetPeriod)
      if (!plan) return
  ...
  return plans.sort((a, b) => a.plannedDate.getTime() - b.plannedDate.getTime())
}
...
Deno.serve(async req => {
  ...
  try {
    ...
    return jsonResponse(200, {
      ...
      note: "Jalankan fungsi ini setiap pukul 12:00 WIB melalui cron Supabase untuk mengecek jadwal pengadaan."
    })
  } catch (error) {
